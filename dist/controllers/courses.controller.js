...
exports.getCourseWithWishlist = exports.deleteCourse = exports.updateCourse = exports.createCourse = exports.getCourseById = exports.listCourses = void 0;
const client_1 = require("@prisma/client");
const errorHandler_middleware_1 = require("../middleware/errorHandler.middleware");
const prisma = new client_1.PrismaClient();
// ─── List Courses (Public) ────────────────────────────────────────────────────
exports.listCourses = (0, errorHandler_middleware_1.asyncHandler)(async (req, res) => {
    const { page = '1', limit = '12', search = '', status = 'ACTIVE', category = '' } = req.query;
...
exports.getCourseById = (0, errorHandler_middleware_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
...
// ─── Create Course (Admin) ────────────────────────────────────────────────────
exports.createCourse = (0, errorHandler_middleware_1.asyncHandler)(async (req, res) => {
...
// ─── Update Course (Admin) ────────────────────────────────────────────────────
exports.updateCourse = (0, errorHandler_middleware_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const { title_ar, title_en, description_ar, description_en, price, duration, image, status, category, maxStudents, startDate, endDate } = req.body;
    const courseDates = parseCourseDates(startDate, endDate);
...
function parseCourseDates(startDate, endDate) {
    const data = {};
    if (startDate !== undefined)
        data.startDate = startDate ? new Date(String(startDate)) : null;
    ...
    if (data.startDate && data.endDate && data.startDate > data.endDate) {
        throw (0, errorHandler_middleware_1.createError)('Course end date must be after its start date.', 422, 'INVALID_COURSE_DATES');
    }
    return data;
}
// ─── Delete Course (Admin) ────────────────────────────────────────────────────
exports.deleteCourse = (0, errorHandler_middleware_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const existing = await prisma.course.findUnique({ where: { id } });
    if (!existing)
        throw (0, errorHandler_middleware_1.createError)('Course not found.', 404, 'COURSE_NOT_FOUND');
...
exports.getCourseWithWishlist = (0, errorHandler_middleware_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
