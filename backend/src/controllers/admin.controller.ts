...
export const updateAcademyProfile = asyncHandler(async (req: Request, res: Response) => {
  const allowedFields = [
...
export const getAnalytics = asyncHandler(async (_req: Request, res: Response) => {
  const [
    ...
    confirmedEnrollments, pendingEnrollments, cancelledEnrollments,
    recentEnrollments, recentUsers, topCourses,
  ] = await Promise.all([
...
export const deleteUser = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!user) { res.status(404).json({ success: false, message: 'User not found.', errorCode: 'USER_NOT_FOUND' }); return; }
  if (user.role === 'ADMIN') { res.status(400).json({ success: false, message: 'Admin accounts cannot be deleted from this screen.', errorCode: 'ADMIN_DELETE_BLOCKED' }); return; }
...
export const exportData = asyncHandler(async (req: Request, res: Response) => {
  const { type = 'enrollments' } = req.query as { type: string };
